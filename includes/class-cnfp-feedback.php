...
class CNFP_Feedback {
	...
	public function goodbye_ajax() {
		...
		if ( is_array( $form['options'] ) ) {
			...
			$html .= '<input type="checkbox" name="epsilon-deactivate-tracking" checked="" id="allow-tracking" value="yes"> <label for="allow-tracking">' . esc_html__( 'Allow us to get more information in order to improve our plugin', 'colorlib-404-customizer' ) . '</label><br>';
		...
		$html .= '<div class="epsilon-deactivate-form-footer"><p><a id="epsilon-deactivate-plugin" href="#">' . __( 'Just Deactivate', 'colorlib-404-customizer' ) . '</a><a id="epsilon-deactivate-submit-form" class="button button-primary" href="#">' . __( 'Submit and Deactivate', 'colorlib-404-customizer' ) . '</a></p></div>'
		?>
		...
		<script>
            jQuery(document).ready(function ($) {
                var deactivateURL = $("#epsilon-deactivate-link-<?php echo esc_attr( $this->plugin_name ); ?>"),
                    ...
                    detailsStrings = {
                        'setup': '<?php echo __( 'What was the dificult part ?', 'colorlib-404-customizer' ) ?>',
                        'documentation': '<?php echo __( 'What can we describe more ?', 'colorlib-404-customizer' ) ?>',
                        'features': '<?php echo __( 'How could we improve ?', 'colorlib-404-customizer' ) ?>',
                        'better-plugin': '<?php echo __( 'Can you mention it ?', 'colorlib-404-customizer' ) ?>',
                        'incompatibility': '<?php echo __( 'With what plugin or theme is incompatible ?', 'colorlib-404-customizer' ) ?>',
                    };
	...
	public function get_form_info() {
		$form            = array();
		$form['heading'] = __( 'Sorry to see you go', 'colorlib-404-customizer' );
		$form['body']    = __( 'Before you deactivate the plugin, would you quickly give us your reason for doing so?', 'colorlib-404-customizer' );
		$form['options'] = array(
			'setup'           => __( 'Set up is too difficult', 'colorlib-404-customizer' ),
			'documentation'   => __( 'Lack of documentation', 'colorlib-404-customizer' ),
			'features'        => __( 'Not the features I wanted', 'colorlib-404-customizer' ),
			'better-plugin'   => __( 'Found a better plugin', 'colorlib-404-customizer' ),
			'incompatibility' => __( 'Incompatible with theme or plugin', 'colorlib-404-customizer' ),
		);
		$form['details'] = __( 'How could we improve ?', 'colorlib-404-customizer' );
